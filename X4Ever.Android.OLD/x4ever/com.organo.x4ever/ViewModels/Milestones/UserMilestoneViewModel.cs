...
namespace com.organo.x4ever.ViewModels.Milestones
{
    public class UserMilestoneViewModel : BaseViewModel
    {
        ...
        public UserMilestoneViewModel(INavigation navigation = null) : base(navigation)
        {
            ...
            CurrentWeightMaximumValue = _converter.DisplayWeightVolume(App.Configuration.AppConfig.MAXIMUM_CURRENT_WEIGHT);
        ...
        {
            UserTrackers = await _trackerService.GetUserTrackerAsync();
            ...
            UserMetas = await _metaService.GetMetaAsync(new string[]
        ...
        private async Task SaveTrackerAsync()
        {
            ...
            if (await Validate())
            {
                var response = await _trackerService.SaveTrackerAsync(await BuildTracker());
        ...
        {
            if (IsGenderRequired)
                await _metaService.SaveMetaAsync(new List<Meta>()
                {
                    await _metaService.AddMeta(GenderSelected.ToString(), MetaConstants.GENDER,
        ...
        private async Task<List<Models.User.Tracker>> BuildTracker()
        {
            ...
            {
                trackerList.Add(await _trackerService.AddTracker(TrackerConstants.CURRENT_WEIGHT,
                    CurrentWeightValue.ToString()));
                if (GoalAchieved)
                {
                    trackerList.Add(await _trackerService.AddTracker(TrackerConstants.TSHIRT_SIZE,
                        TShirtSize.Trim()));
                    trackerList.Add(await _trackerService.AddTracker(TrackerConstants.FRONT_IMAGE,
                        ImageFront.Trim()));
                    trackerList.Add(await _trackerService.AddTracker(TrackerConstants.SIDE_IMAGE,
                        ImageSide.Trim()));
                    trackerList.Add(await _trackerService.AddTracker(TrackerConstants.ABOUT_JOURNEY,
        ...
        private async Task<bool> Validate()
        {
            ...
            await Task.Run(() =>
            {
                ...
                else if (CurrentWeightValue < _converter.DisplayWeightVolume(App.Configuration.AppConfig.MINIMUM_CURRENT_WEIGHT))
                    validationErrors.Add(string.Format(TextResources.Validation_MustBeMoreThan,
                        TextResources.WeightLossGoal, _converter.DisplayWeightVolume(App.Configuration.AppConfig.MINIMUM_CURRENT_WEIGHT)));
        ...
        private Milestone achievedMilestone;
        public const string AchievedMilestonePropertyName = "AchievedMilestone";

        public Milestone AchievedMilestone
        {
