...
namespace com.organo.x4ever.ViewModels.Profile
{
    public class SettingsViewModel : BaseViewModel
    {
        public SettingsViewModel(INavigation navigation = null) : base(navigation)
        {
            ...
            GetCountryList().GetAwaiter();
            TabSelected(TabTitle.EditProfile);
        }
        ...
        public async void TabSelected(TabTitle tabTitle)
        {
            Selected = tabTitle;
            switch (tabTitle)
            {
        ...
        {
            if (UserMeta == null)
                UserMeta = await DependencyService.Get<IMetaService>().GetLastestMetaAsync(App.CurrentUser.UserInfo.ID);
            if (UserMeta != null)
            {
        ...
        private async void SaveWeightVolume(string weightVolumeType)
        {
            ...
            var response = await DependencyService.Get<IUserSettingService>().UpdateUserWeightVolumeAsync(requestModel);
            if (response == HttpConstants.SUCCESS)
            {
